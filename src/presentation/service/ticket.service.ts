...
export class TicketService {
  ...
  public get pendingTickets(): Ticket[] {
    return this.tickets.filter((ticket) => !ticket.handleAtDestk);
  }
  ...
  public drawTicket(desk: string) {
    // SI NO HAY NULO
    console.log(desk);
    const ticket = this.tickets.find((ticket) => !ticket.handleAtDestk);
    if (!ticket) {
      return { status: "error", message: "No hay tickets pendientes" };
    }
    ticket.handleAtDestk = desk;
